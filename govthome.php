<?php
	...
	$filter = "  ";
		...
		$sql = "SELECT  count(*) as count1, c_pincode FROM candidates WHERE c_id IN ({$filter}) GROUP BY c_pincode";
		...
		if($result->num_rows>0)
		{
			...
			{
				$arr_pincodes[$count] = $row['c_pincode'];
...
<head>
	...
	<script>
		...
		{
			var x = document.forms["myForm"]["selectDocument1"].value;
			var flag = true;
			if (x == null || x == "None") 
			{
  ...
  overflow-y:scroll;">
				  ...
				  <form name="myForm" action="" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
				   <div class="intro-text">
						<span class="skills">Analysis Notification</span>
                        <hr class="star-light">
							...
							<table>
								...
								while($i<$count3)
								{
									...
									<tr>
										...
										<td>{$arr_c3_pincodes[$i]}-{$row['p_area']}</td>
						...
						<div id="analysis">
							...
							<table>
								...
								while($i<$count2)
								{
									...
									<tr>
										<td style=\"text-align:center;\">{$j}</td>
										<td>{$arr_c2_pincodes[$i]}-{$row['p_area']}</td>
						...
						<div id="analysis">
							...
							<table>
								...
								while($i<$count1)
								{
									...
									<tr>
										<td style=\"text-align:center;\">{$j}</td>
										<td>{$arr_c1_pincodes[$i]}-{$row['p_area']}</td>
